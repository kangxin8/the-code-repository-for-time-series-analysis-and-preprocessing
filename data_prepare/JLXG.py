'''
该脚本用于将基于JLXG构建的数据集以样本和标签的列表形式返回
关于JLXG数据的一些关键信息
1、采样频率统一为8K,为发生故障后的数据，样本的长度为18000，一个文件对应个17个长度为1024的样本
'''
import json
...
def file_list_obtain(mode='Health', number_of_files=20, dir_path=r'F:\OneDrive - hnu.edu.cn\项目\青岛儒海\项目代码\data\data_建龙西钢_8.2-3\泵驱动端'):
    file_name_list = read_file_name_extension(root_path=dir_path)  # 目录下所有文件名称形成的列表
    if mode == 'Health':
        return file_name_list[51:71]
    else:
        return file_name_list[0:5]
...
def STFT_time_frequency(sample, fs=12e3, J=10, Q=(11, 1), non_threshod=95):
    ...
    -------
    '''
    STFT_analysis = STFT_TF_analysis(fs=fs, nperseg=128, noverlap=125)  # 实例化小波散射对象
...
def from_index_to_filepath(index=105, root_path=r'D:\OneDrive - hnu.edu.cn\项目\青岛儒海\项目代码\data\data_建龙西钢_8.2-3\泵驱动端'):
    '''
    ...
    指定索引的文件的绝对路径
    '''
    dir_list = [root_path]  # 根目录下所有文件夹的绝对路径
    file_path_list = []
    for dir in dir_list:
...
def filepath_to_samplelist(filepath, sample_len=2048, index=4, label=None, signal_process_method=None, params=None):
    ...
    fl = np.array(data[desired_key]).flatten()
    fl = fl.reshape(-1, )
    data = []
    lab = []
...
class JLXG(object):
    ...
    def original_1d(self):
        # class1 只有故障样本，故障样本标签为0
        file_list1 = file_list_obtain(mode='F', number_of_files=1, dir_path=r'F:\OneDrive - hnu.edu.cn\项目\青岛儒海\项目代码\data\data_建龙西钢_8.2-3\泵驱动端') # 要读取的同类文件（文件具有相同的类型，相同的采样频率）,20K
        sample_list1, label_list1 = dataset_construct(files=file_list1, sample_len=2560, index=3, label=1, signal_process_method='original_1d')  # 将列表中的文件转换为样本
        # 将所有的样本组合起来
    ...
    def STFT(self):
        # class1 只有故障样本，故障样本标签为0
        params = CWT_Params(fs=8e3, J=10, Q=(11, 1), non_threshod=95)
        file_list1 = file_list_obtain(mode='F', number_of_files=1,
                                      dir_path=r'F:\OneDrive - hnu.edu.cn\项目\青岛儒海\项目代码\data\data_建龙西钢_8.2-3\泵驱动端')  # 要读取的同类文件（文件具有相同的类型，相同的采样频率）,20K
    ...
    def CWT(self):
        ...
        file_list1 = file_list_obtain(mode='F', number_of_files=1, dir_path=r'F:\OneDrive - hnu.edu.cn\项目\青岛儒海\项目代码\data\data_建龙西钢_8.2-3\泵驱动端') # 要读取的同类文件（文件具有相同的类型，相同的采样频率）,20K
        params = CWT_Params(fs=8e3, J=10, Q=(11, 1), non_threshod=97)
        logging.info('SCWT_params:{}'.format(params))
        sample_list1, label_list1 = dataset_construct(files=file_list1, sample_len=self.sample_len, index=3, label=0,
                                                      signal_process_method='CWT', SCWT_params=params)  # 将列表中的文件转换为样本


        logging.info('number_of_fault_class:{}'.format(len(sample_list1)))
