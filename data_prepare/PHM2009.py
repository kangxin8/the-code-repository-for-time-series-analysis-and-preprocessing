'''
该脚本编写一个PHM20009数据集数据读取对象，用于训练集、测试集的构建以及样本的分析

数据集基本信息
dir:E:\datasets\PHM\PHM2009齿轮\PHM2009 Gearbox\PHM_Society_2009_Competition_Expanded_txt
数据集说明文件：
fs:66.7K
...
def file_list_obtain(mode='Health', number_of_files=20, dir_path=r'E:\datasets\辛辛那提\IMS\2nd_test'):
...
def from_index_to_filepath(index=105, root_path=r'E:\datasets\西安交通大学齿轮箱数据\XJTU_Gearbox'):
    '''
...
def filepath_to_samplelist(args, file=None, label=None):
    '''
    ...
    返回：以列表形式返回
    '''
    data = np.loadtxt(file, skiprows=15, usecols=1)  # 读取文件内容
    fl = data.flatten()
    data = []
    lab = []
...
def dataset_construct(args, label=1, file_info=None, channel=None):
    ...
    for file_path in file_path_list:
        data, lab = filepath_to_samplelist(args, file=file_path, label=label)
...
class PHM2009(object):
    def __init__(self, args, sample_len=4096, file_name=None):
        self.args = args
    ...
    def single_file_samplelist(self):
        sample_list, label_list = dataset_construct(self.args, label=0)  # 将列表中的文件转换为样本
...
if __name__ == '__main__':
    # data = np.loadtxt(r'E:\datasets\西安交通大学齿轮箱数据\XJTU_Gearbox\1ndBearing_ball\Data_chan1.txt', skiprows=13)  # 读取文件内容
    # ====修改文件名称=======
    # ======获取所有需要更改的文件夹绝对路径======
    root = r'E:\datasets\西安交通大学齿轮箱数据\XJTU_Gearbox'  # 上级文件夹路径
    parent_dir_list = read_obsolute_path_dir(root)  # 该文件夹下所有的文件夹的绝对路径
    for dir in parent_dir_list:
        add_folder_names_as_prefix(dir)
